...
export default function NoticeProvider({ children, initialNotices }: Props) {
  ...
  const addNotice = async ({
    ...
    try {
      const { res, data } = await clientApi.postNotice({ title, body });
      if (res.status !== 200) {
  ...
  const updateNotice = async ({
    ...
    try {
      const { response, data } = await clientApi.patchNotice({
        _id,
        title,
        body,
      });
